...
CREATE TABLE IF NOT EXISTS tiktok_tokens (id INTEGER PRIMARY KEY CHECK(id=1), access_token TEXT NOT NULL, refresh_token TEXT, expires_at INTEGER, refresh_expires_at INTEGER, open_id TEXT, scope TEXT, updated_at INTEGER NOT NULL);
`);
...
const saveTokens = (data) => db.prepare(`INSERT INTO tiktok_tokens(id,access_token,refresh_token,expires_at,refresh_expires_at,open_id,scope,updated_at) VALUES(1,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET access_token=excluded.access_token,refresh_token=excluded.refresh_token,expires_at=excluded.expires_at,refresh_expires_at=excluded.refresh_expires_at,open_id=excluded.open_id,scope=excluded.scope,updated_at=excluded.updated_at`).run(
  encrypt(data.access_token), encrypt(data.refresh_token || ''), data.expires_in ? Date.now() + Number(data.expires_in) * 1000 : null,
  data.refresh_expires_in ? Date.now() + Number(data.refresh_expires_in) * 1000 : null, data.open_id || '', data.scope || '', Date.now()
);
...
async function getTikTokToken() {
  ...
  if (access && expiresAt > Date.now() + 15 * 60 * 1000) return access;
  const refresh = decrypt(row.refresh_token);
  if (!refresh || !process.env.TIKTOK_CLIENT_KEY || !process.env.TIKTOK_CLIENT_SECRET) return access;
  const body = new URLSearchParams({ client_key: process.env.TIKTOK_CLIENT_KEY, client_secret: process.env.TIKTOK_CLIENT_SECRET, grant_type: 'refresh_token', refresh_token: refresh });
...
app.get('/api/memories', (req, res) => {
  ...
  if (!sid) return res.json({ memories: [] });
  res.json({ memories: db.prepare("SELECT id,role,content,created_at FROM memories WHERE session_id=? ORDER BY id DESC LIMIT 50").all(sid).reverse() });
});
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const rows = db.prepare("SELECT role,content FROM memories WHERE session_id=? AND role IN ('user','assistant') ORDER BY id DESC LIMIT 24").all(sid).reverse();
...
app.post('/api/memory', (req, res) => {
  const sid = clean(req.body.sessionId, 120), content = clean(req.body.content, 2000);
  if (!sid || !content) return res.status(400).json({ error: 'sessionId and content required' });
  db.prepare('INSERT INTO memories(session_id,role,content) VALUES(?,?,?)').run(sid, 'memory', content);
  res.json({ ok: true });
});
app.delete('/api/memories', (req, res) => {
  ...
  db.prepare('DELETE FROM memories WHERE session_id=?').run(sid);
  res.json({ ok: true });
});
...
app.get('/api/tiktok/creator-info', async (_, res) => {
  try {
    ...
    if (!token) return res.status(401).json({ error: 'TikTok is not connected or the token could not be refreshed.' });
    ...
    const data = await r.json(); res.status(r.ok ? 200 : 502).json(data);
  } catch (e) { res.status(502).json({ error: e?.message || 'TikTok creator info failed' }); }
});
...
app.post('/api/tiktok/post-url', async (req, res) => {
  try {
    const token = await getTikTokToken();
    if (!token) return res.status(401).json({ error: 'TikTok is not connected or the token could not be refreshed.' });
    const videoUrl = clean(req.body.videoUrl, 2000), title = clean(req.body.title, 150);
    if (!/^https:\/\//i.test(videoUrl)) return res.status(400).json({ error: 'videoUrl must be HTTPS.' });
    const body = { post_info: { title, privacy_level: clean(req.body.privacyLevel || 'SELF_ONLY', 40), disable_duet: Boolean(req.body.disableDuet), disable_comment: Boolean(req.body.disableComment), disable_stitch: Boolean(req.body.disableStitch) }, source_info: { source: 'PULL_FROM_URL', video_url: videoUrl } };
